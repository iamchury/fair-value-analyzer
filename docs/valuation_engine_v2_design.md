...
The engine uses completed daily SOXX price history and calculates EMA5, EMA10,
EMA15, EMA20, EMA50, a rolling prior high, drawdown, and short EMA convergence.
The EMA formula is pandas-compatible:
`ewm(span=N, adjust=False, min_periods=N).mean()`. The price field preference is
Adjusted Close first, then Close, while preserving the actual field used in the
result. The previous SMA-based SOXX implementation was replaced by EMA.
...
Persistent relative position is not a new crossover. If EMA5 remains above
EMA10 after a prior cross, the UI should display the position as EMA5 ABOVE
EMA10 or NO NEW CROSS, not as a new BUY signal.

Graded buy signals:

- BUY: EMA5 crosses above EMA10.
- STRONG_BUY: EMA5 crosses above EMA15.
- VERY_STRONG_BUY: EMA5 crosses above EMA20.

Graded sell signals:

- SELL: EMA5 crosses below EMA10.
- STRONG_SELL: EMA5 crosses below EMA15.
- VERY_STRONG_SELL: EMA5 crosses below EMA20.
...
rolling prior high, EMA5/EMA15/EMA20 remain below EMA50, and EMA5 crosses above
EMA15. If EMA5 also crosses above EMA20, the primary signal becomes
VERY_STRONG_BUY.

The convergence sell setup activates when EMA5, EMA10, EMA15, and EMA20 are
within the configured spread threshold above EMA50, followed by EMA5 crossing
below EMA15. If EMA5 also crosses below EMA20, the primary signal becomes
VERY_STRONG_SELL.
...
The Streamlit SOXX chart displays the latest 100 trading days only. Indicators,
signals, prior highs, and event history are calculated from the full configured
history before chart truncation, so EMA warm-up and 252-trading-day prior highs
are not recalculated from the visible window.
